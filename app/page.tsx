...
export default async function HomePage() {
  ...
  let recent: Awaited<ReturnType<typeof getHomeRecentEpisodes>> = [];
  let showCount = 0;
  let episodeCount = 0;
  let catalogProbe: Awaited<ReturnType<typeof probeCatalogBackend>> = "missing_env";
  try {
    [featured, recent, showCount, episodeCount, catalogProbe] = await Promise.all([
      getFeaturedShows(HOMEPAGE_FEATURED_LIMIT),
      getHomeRecentEpisodes(HOMEPAGE_RECENT_EPISODES),
      getActiveShowCount(),
      getPublicEpisodeCount(),
  ...
  return (
    <main>
      ...
      <section className="container-shell border-t border-line/40 py-12">
        <div className="card border-accent/20 bg-soft/20 p-8">
      ...
      <section id="featured" className="container-shell py-10">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold">Hand-picked teaching</h2>
          ...
          <Link href="/explore" className="shrink-0 text-sm font-medium text-amber-200 hover:text-white">
            Browse all sources →
          </Link>
        </div>
      ...
      <section className="container-shell py-14">
        <div className="mb-8">
          ...
          <h2 className="mt-2 text-3xl font-semibold">Newest episodes</h2>
          <p className="mt-2 max-w-2xl text-sm text-muted">
