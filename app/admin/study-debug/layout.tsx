...
export const metadata = {
  title: "Study Debug · Admin",
  robots: { index: false, follow: false },
};

export const dynamic = "force-dynamic";

export default async function AdminStudyDebugLayout({ children }: { children: ReactNode }) {
  let user = null;
  try {
    ...
    return (
      <main className="container-shell max-w-lg py-16">
        ...
        <p className="mt-2 text-sm text-muted">
          Set <code className="text-slate-400">FEEDBACK_ADMIN_EMAILS</code> in the server environment (same allowlist as other admin tools), then redeploy.
        </p>
      </main>
    );
  }
