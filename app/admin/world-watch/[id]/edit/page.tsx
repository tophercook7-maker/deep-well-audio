...
export default async function AdminWorldWatchEditPage(ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin
    ...
    .select(
      "id, created_at, updated_at, published_at, title, slug, source_name, source_url, image_url, external_image_url, summary, reflection, category, is_published, source_type, source_feed, source_guid, canonical_url, pinned, pinned_rank, ingestion_status, public_teaser, member_commentary, scripture_refs, discernment_notes, key_takeaways"
    )
    .eq("id", id)
    .maybeSingle();
